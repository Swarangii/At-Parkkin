...
const Sidebar = ({ selectedMenu, setSelectedMenu }) => {
  const menuItems = [
    ...
    { id: "reports", icon: "📊", label: "Reports", path: "/vendor-report" },
    // { id: "vouchers", icon: "🎫", label: "Vouchers" },
    { id: "users", icon: "👤", label: "Users", path: "/user-detail" },
    { id: "settings", icon: "⚙️", label: "Settings", path: "/vendor-settings" },
  ];

  return (
    <div className="sidebar-vender">
      <div className="logo-section-vender">
        <img src={ImgLogo} alt="Eways Services" className="logo-vender" />
        <span className="brand-name-vender">At Parkkin</span>
      </div>

      <nav className="nav-menu-vender">
        {menuItems.map((item) => (
          <NavLink
            key={item.id}
            to={item.path}
            ...
            className={({ isActive }) =>
              `menu-item-vender ${isActive ? "active" : ""}`
            }
